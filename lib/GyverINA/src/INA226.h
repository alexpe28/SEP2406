...
#define INA226_MASK_ENABLE_ADDR 0x06    // This register sets the alert configuration and conversion ready flag.
#define INA226_ALERT_ADDR 0x07          // This register contains the limit value to compare to the selected alert function.
...
class INA226 {
   public:
    ...
    bool begin(int __attribute__((unused)) sda = 0, int __attribute__((unused)) scl = 0) {
        ...
        if (sda || scl) Wire1.begin(sda, scl);  // Инициализация для ESP
        else Wire1.begin();
#else
        Wire1.begin();  // Инициализация шины I2c
#endif
   ...
   private:
    ...
    void writeRegister(uint8_t address, uint16_t data) {
        Wire1.beginTransmission(_iic_address);  // Начинаем передачу
        Wire1.write(address);                   // Отправляем адрес
        Wire1.write(highByte(data));            // Отправляем старший байт
        Wire1.write(lowByte(data));             // Отправляем младший байт
        Wire1.endTransmission();                // Заканчиваем передачу
    }
    ...
    uint16_t readRegister(uint8_t address) {
        ...
        Wire1.write(address);                         // Отправляем адрес
        Wire1.endTransmission();                      // Заканчиваем передачу
        Wire1.requestFrom(_iic_address, (uint8_t)2);  // Запрашиваем 2 байта
        return Wire1.read() << 8 | Wire1.read();       // Клеим и возвращаем результат
    }

    // Проверка присутствия
    bool testConnection(void) {
        Wire1.beginTransmission(_iic_address);  // Начинаем передачу
        return (bool)!Wire1.endTransmission();  // Сразу заканчиваем, инвертируем результат
    }
